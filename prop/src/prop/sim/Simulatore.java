...
public class Simulatore {
	...
	public Bianca creaPersona() {
		return new Bianca(this.ambiente);
	}

	public List<Bianca> getPersona() {
	...
	private void eseguiPassoDellaSimulazione() {
		final List<Bianca> persone = new ArrayList<>(this.ambiente.getAllPersone());
		for(Bianca p : persone) // prima muovo tutti 
			p.mossa(); 
		for(Bianca persona : persone) { // poi calcolo le statistiche
			final Set<Bianca> stessaPosizione = this.getAmbiente().getPersona(persona.getPosizione());
